...
async def handle_track(update: Update, context: ContextTypes.DEFAULT_TYPE, track_info: dict, send_wait_message: bool = True, send_cover: bool = True) -> None:
    messages_to_delete = []
    
    if send_wait_message:
        proccessing_message = await update.message.reply_text(MessageHandler().get_message("processing_message"))
        messages_to_delete.append(proccessing_message.message_id)
    
    if send_cover:
        cover_photo_url = track_info['album']['images'][0]['url']
        caption = MessageHandler().get_message("track_caption", track_name=track_info['name'], artist_name=track_info['artists'][0]["name"], album_name=track_info['album']['name'], release_date=track_info["album"]['release_date'])
...
async def download_spotify_track(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
async def download_spotify_playlist(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    caption = MessageHandler().get_message("playlist_caption", playlist_name=playlist_info['name'], owner_name=playlist_info['owner']['display_name'], total_tracks=playlist_info['tracks']['total'])
    await update.message.reply_photo(photo=cover_photo_url, caption=caption)
    for track in playlist_info['tracks']['items']:
        if track['track']['is_local'] or track['track']['track'] is not True:
            continue
        track_info = SpotifyService().get_track_info(track['track']['id'])
        await handle_track(update, context, track_info, send_wait_message=False, send_cover=False)

async def download_spotify_album(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    caption = MessageHandler().get_message("album_caption", album_name=album_info['name'], artist_name=album_info['artists'][0]["name"], release_date=album_info['release_date'])
    await update.message.reply_photo(photo=cover_photo_url, caption=caption)
