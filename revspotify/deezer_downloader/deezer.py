...
def init_deezer_session():
    ...
    session.cookies.update({'arl': Config.DEEZER_ARL, 'comeback': '1'})
...
def download_song(song, unique_name_suffix=None, output_file=None):
    ...
    trackname = song["SNG_TITLE"]
    trackname = trackname.replace("/","-").replace("\\","-")
    if output_file is None:
        if unique_name_suffix is not None:
            output_file = f"static/songs/{trackname}-{unique_name_suffix}.mp3"
        else:
            output_file = f"static/songs/{trackname}.mp3"
    ...
    try:
        ...
        return {"music_path": output_file}



def download_cover(song, unique_name_suffix=None, output_file=None):
    # downloads and decrypts the song from Deezer. Adds ID3 and art cover
    # song: dict with information of the song (grabbed from Deezer.com)
    # output_file: absolute file name of the output file
    assert type(song) == dict, "song must be a dict"
    ...
    if output_file is None:
        if unique_name_suffix is not None:
            output_file = f"static/covers/{trackname}-{unique_name_suffix}.png"
        else:
            output_file = f"static/covers/{trackname}.png"
    assert type(output_file) == str, "output_file must be a str"
    


    for image_quality in [1200, 1000, 800, 500, 400]:
        alb_picture = song["ALB_PICTURE"]
        url = f"https://cdns-images.dzcdn.net/images/artist/{alb_picture}/{image_quality}x{image_quality}.jpg"
        r = requests.get(url, allow_redirects=True)
        if r.status_code == 200:
            open(output_file, 'wb').write(r.content)
            return {"cover_path": output_file}
...
def get_song_infos_from_deezer_website(id, search_type=TYPE_TRACK):
...
def deezer_search(search, search_type=TYPE_TRACK):
    ...
    if search_type == TYPE_ALBUM_TRACK:
        resp = get_song_infos_from_deezer_website(search, TYPE_ALBUM)
    else:
    ...
    for item in resp:
        ...
        if search_type == TYPE_TRACK:
            ...
            i['cover_xl'] = item['album']['cover_xl']
...
def test_deezer_login():
    ...
    try:
        song = get_song_infos_from_deezer_website("917265", TYPE_TRACK)
