...
def test_query_track_link_which_is_on_youtube():
    ...
    assert (
        context_mock.bot.send_audio.call_args_list[0][0][1].read()
        == open(
            "tests/song/youtube/Shaal - The Shawl (Ft.Ehsan Sobhani).mp3", "rb"
        ).read()
    )
    assert len(context_mock.bot.send_audio.call_args_list) == 1
...
def test_query_album():
    ...
    assert (
        context_mock.bot.send_audio.call_args_list[0][0][1].read()
        == open("tests/album/Taunt.mp3", "rb").read()
    )
    assert (
        context_mock.bot.send_audio.call_args_list[1][0][1].read()
        == open("tests/album/One Day.mp3", "rb").read()
    )
    assert (
        context_mock.bot.send_audio.call_args_list[2][0][1].read()
        == open("tests/album/Sex Sells.mp3", "rb").read()
    )
    assert (
        context_mock.bot.send_audio.call_args_list[3][0][1].read()
        == open("tests/album/Cause for Concern.mp3", "rb").read()
    )
...
def test_query_artist():
    ...
    assert (
        context_mock.bot.send_audio.call_args_list[0][0][1].read()
        == open("tests/artist/Another One Bites The Dust.mp3", "rb").read()
    )
    assert (
        context_mock.bot.send_audio.call_args_list[1][0][1].read()
        == open("tests/artist/Bohemian Rhapsody.mp3", "rb").read()
    )
    assert (
        context_mock.bot.send_audio.call_args_list[2][0][1].read()
        == open("tests/artist/Don't Stop Me Now.mp3", "rb").read()
    )
    assert (
        context_mock.bot.send_audio.call_args_list[3][0][1].read()
        == open("tests/artist/Under Pressure.mp3", "rb").read()
    )
    assert (
        context_mock.bot.send_audio.call_args_list[4][0][1].read()
        == open("tests/artist/We Will Rock You.mp3", "rb").read()
    )
    assert (
        context_mock.bot.send_audio.call_args_list[5][0][1].read()
        == open("tests/artist/Crazy Little Thing Called Love.mp3", "rb").read()
    )
    assert (
        context_mock.bot.send_audio.call_args_list[6][0][1].read()
        == open("tests/artist/Radio Ga Ga.mp3", "rb").read()
    )
    assert (
        context_mock.bot.send_audio.call_args_list[7][0][1].read()
        == open("tests/artist/We Are The Champions.mp3", "rb").read()
    )
    assert (
        context_mock.bot.send_audio.call_args_list[8][0][1].read()
        == open("tests/artist/I Want To Break Free.mp3", "rb").read()
    )
    assert (
        context_mock.bot.send_audio.call_args_list[9][0][1].read()
        == open("tests/artist/Somebody To Love.mp3", "rb").read()
    )
